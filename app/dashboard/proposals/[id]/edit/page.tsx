...
import { ProposalEditForm } from "@/components/dashboard/proposal-edit-form"
...
export default async function EditProposalPage({
  ...
  const [
    ...
    { data: proposalServices, error: psError },
    ...
    supabase
      ...
      .select("id, service_id, service_name_pt, service_name_en, pricing_type, quantity, unit_price, custom_price, total_price, notes, sort_order")
      .eq("proposal_id", id)
    ...
    supabase.from("service_included_items").select("*").order("sort_order", { ascending: true }),
    supabase.from("service_priced_options").select("*").order("sort_order", { ascending: true }),
    ...
    supabase.from("terms_templates").select("id, name, content_pt, content_en").order("name", { ascending: true }),
  ])
  ...
  if (servicesError || categoriesError || includedError || optionsError || companyError || clientsError || termsError || psError) {
    return <div>Erro ao carregar dados.</div>
  }

  const proposalServiceIds = (proposalServices ?? []).map((s: Record<string, unknown>) => (s as { id: string }).id)
  let proposalServiceOptions: Record<string, unknown>[] = []
  if (proposalServiceIds.length > 0) {
    const { data } = await supabase
      .from("proposal_service_options")
      .select("id, proposal_service_id, service_priced_option_id, quantity, unit_price, total_price, notes, sort_order")
      .in("proposal_service_id", proposalServiceIds)
      .order("sort_order", { ascending: true })
    proposalServiceOptions = (data ?? []) as Record<string, unknown>[]
  }
    ...
    <div className="flex flex-1 flex-col gap-6 p-6">
      <div>
        ...
        <p className="text-muted-foreground">
          {(p.reference_number as string) ? `Referência: ${p.reference_number}` : "Editar dados da proposta existente."}
        </p>
      </div>
      <ProposalEditForm
        proposalId={id}
        proposal={p}
        proposalServices={(proposalServices ?? []) as Record<string, unknown>[]}
