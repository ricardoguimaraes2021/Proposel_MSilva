...
import { buildContractPdfData } from "@/lib/build-contract-pdf-data"
import type { CompanyForPreview, ProposalApiPayload } from "@/lib/build-proposal-preview-data"
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const attachment =
    ...
    url.searchParams.get("download")?.toLowerCase() === "true"
  ...
  if ((proposal as { status?: string }).status !== "accepted") {
    return NextResponse.json(
      { error: "O contrato só está disponível para propostas marcadas como Aceite." },
      { status: 403 }
    )
  }
  ...
  const contractData = buildContractPdfData(payload, company, lang)

  const pdfBuffer = await renderToBuffer(
    React.createElement(ContractPdfDocument, { data: contractData }) as Parameters<typeof renderToBuffer>[0]
  )

  const filenamePrefix = lang === "en" ? "Contract" : "Contrato"
  const filename = `${filenamePrefix}_${contractData.referenceNumber}.pdf`.replace(/[/\\]/g, "-")
